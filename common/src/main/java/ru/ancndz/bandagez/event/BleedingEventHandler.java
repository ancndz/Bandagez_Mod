...
public class BleedingEventHandler {
    ...
    public static void onPlayerTakesDamage(DamageSource source, LivingEntity entity) {
        ...
        Optional.ofNullable(ENTITY_TYPES_FOR_BLEEDING.get(damageSourceEntity.getType()))
                .ifPresent(bleedingChance -> atomicBoolean
                    .set(applyEffect(entity, bleedingChance, ModMobEffects.BLEEDING.get(), source)));
        if (atomicBoolean.get()) {
            return;
        }
        ...
        Optional.ofNullable(ENTITY_TYPES_FOR_HARD_BLEEDING.get(damageSourceEntity.getType()))
                .ifPresent(bleedingChance -> atomicBoolean
                    .set(applyEffect(entity, bleedingChance, ModMobEffects.HARD_BLEEDING.get(), source)));
    }
    ...
    private static boolean
        ...
        if (entity.level().getRandom().nextFloat() < bleedingChance) {
            entity.addEffect(new MobEffectInstance(effect, MobEffectInstance.INFINITE_DURATION, 0, false, false, true), source.getEntity());
            return true;
        }
        return false;
    }
